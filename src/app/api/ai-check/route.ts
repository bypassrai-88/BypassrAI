...
import { DETECTOR_STRUCTURED_SYSTEM } from "@/lib/prompts";
...
function parseStructuredResponse(raw: string): DetectorResponse | null {
  const cleaned = raw.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "").trim();
  try {
    const parsed = JSON.parse(cleaned) as DetectorResponse;
    ...
    return {
      ...
      flaggedPhrases: Array.isArray(parsed.flaggedPhrases)
        ? parsed.flaggedPhrases.filter((p): p is string => typeof p === "string")
        : [],
    };
  } catch {
    return null;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 512,
      system: DETECTOR_STRUCTURED_SYSTEM,
      messages: [{ role: "user", content: text }],
    });

    const block = message.content.find((b) => b.type === "text");
    const raw = block && block.type === "text" ? block.text.trim() : "";
    const parsed = raw ? parseStructuredResponse(raw) : null;

    if (!parsed) {
      return NextResponse.json(
        { error: "Could not parse AI detection result." },
        { status: 502 }
      );
    }
    ...
    const response = NextResponse.json({
      ...
      result: `${parsed.overallScore}% AI — ${summary}`,
    });
    if (!user) {
      const quota = await checkAnonymousQuota(request, wordCount);
