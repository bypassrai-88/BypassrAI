...
export async function POST(request: NextRequest) {
  ...
  if (!priceId || typeof priceId !== "string" || !priceId.startsWith("price_")) {
    return NextResponse.json(
      { error: `Invalid or missing Stripe Price ID for plan "${plan}". In .env.local set STRIPE_${plan.toUpperCase()}_PRICE_ID to the Price ID from Stripe Dashboard (e.g. price_1ABC...).` },
      { status: 500 }
    );
  }
  ...
  // If user already has an active or trial subscription, send them to the billing portal to change plan
