...
import { getTranslateSystem } from "@/lib/prompts";
...
function isValidTargetLanguage(
  value: unknown
): value is (typeof ALLOWED_TARGET_LANGUAGES)[number] {
  return (
    typeof value === "string" &&
    ALLOWED_TARGET_LANGUAGES.includes(value as (typeof ALLOWED_TARGET_LANGUAGES)[number])
  );
}
...
export async function POST(request: NextRequest) {
  ...
  const targetLanguage = isValidTargetLanguage(body.targetLanguage)
    ? body.targetLanguage
    : "English";
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 1024,
      system: getTranslateSystem(targetLanguage),
